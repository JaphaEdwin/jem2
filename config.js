...
// ─── Local overrides (for Admin Panel preview + GitHub Pages) ───
try {
  const saved = localStorage.getItem('JEM_CONFIG');
  if (saved) {
    const parsed = JSON.parse(saved);
    // shallow merge for safety
    CONFIG = { ...CONFIG, ...parsed, site: { ...CONFIG.site, ...(parsed.site || {}) }, seo: { ...CONFIG.seo, ...(parsed.seo || {}) } };
  }
} catch (e) {
  console.warn('Could not load saved JEM_CONFIG from localStorage:', e);
}
